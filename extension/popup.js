...
// ── Config ───────────────────────────────────────────────────────────────────
const KURYANA = "https://mdl.dramatrackr.fr";
...
async function fetchPersonalSchedule() {
    ...
    try {
        ...
        console.log(
            "[Drama Calendar] Personal entries mapped:",
            result.length,
            "date range:",
            result[0]?.airDate,
            "→",
            result[result.length - 1]?.airDate,
        );
        return result;
    } catch (err) {
...
function renderFeed() {
    ...
    byDate.forEach((dayMap, dateStr) => {
        ...
        ${
            isPersonal
                ? `<button class="check-btn" title="Mark as watched">
...
async function initData(forceRefresh = false) {
    ...
    try {
        ...
        document.getElementById("agenda-feed").innerHTML = `<div class="error-state">⚠ ${err.message}</div>`;
    } finally {
        loading = false;
