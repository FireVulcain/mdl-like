...
/**
 * Character stills from asianwiki, fetched from the reader's own browser —
 * asianwiki's Cloudflare turns every server away and lets a browser
 * through. Two callers share this: the popup's button, which walks every
 * Korean chart the app has, and the content script on the app's own pages,
 * which does one chart the moment it is written (see content.js). Both find
 * the asianwiki page by search, read the cast tables (a row of photos, a
 * row of actors, a row of characters) and post them to the app, which
 * matches actors to the chart's people and keeps the photos as stills.
 *
 * `fetchHtml(url)` is how the caller reaches asianwiki: the popup fetches
 * directly, the content script asks the service worker (a content script's
 * own fetch is held to the page's CORS, which asianwiki does not allow). It
 * resolves to { ok, status, url, text }.
 */
const TrackrStills = (() => {
    ...
    async function searchPages(fetchHtml, title, year, listOnly = false) {
        ...
        const res = await fetchHtml(`${AW}/index.php?search=${encodeURIComponent(title)}&fulltext=${listOnly ? 1 : 0}`);
    ...
    async function fetchPage(fetchHtml, page) {
        const res = await fetchHtml(`${AW}/${encodeURIComponent(page.replace(/ /g, "_"))}`);
    ...
    /**
     * One chart: finds its asianwiki page, posts the cast rows to the app.
     * Resolves to { page, matched, people, unmatched, unused } when a page
     * took, or { error, seen } when none did.
     */
    async function stillsForChart(fetchHtml, appUrl, chart) {
        let seen = [], lastError = null;
        for (const variant of titleVariants(chart.title, chart.asianwiki)) {
            let found = await searchPages(fetchHtml, variant, chart.year);
            ...
            if (found.landed && castRows(found.html).length === 0) {
                const listed = await searchPages(fetchHtml, variant, chart.year, true);
            ...
            for (const page of found.pages) {
                const html = found.html && found.pages[0] === page ? found.html : (await fetchPage(fetchHtml, page)).html;
                ...
                const res = await fetch(`${appUrl}/api/ext/character-maps/stills`, {
                    ...
                    body: JSON.stringify({ mdlSlug: chart.mdlSlug, page, rows }),
                });
                const out = await res.json();
                if (!res.ok) { lastError = `app ${res.status} ${out.error ?? ""}`; continue; }
                return { page, rows: rows.length, ...out };
            }
        }
        return { error: lastError ?? "no exact title", seen: [...new Set(seen)] };
    }

    /** The popup's bulk run: every chart without a still, or all of them. */
    async function runStills(fetchHtml, appUrl, redo, report) {
        ...
        for (const c of todo) {
            ...
            try {
                const out = await stillsForChart(fetchHtml, appUrl, c);
                // Missing faces with no unused rows: the page has no photo for
                // them. Missing faces beside unused rows: a name the fold missed.
                if (out.error) line(`${c.title}: ${out.error}${out.seen.length ? ` (search saw: ${out.seen.slice(0, 4).join(" · ")})` : ""}`);
                else line(`${c.title} → ${out.page}: ${out.matched}/${out.people} faces from ${out.rows} photos${out.unmatched.length ? ` — missing ${out.unmatched.join(", ")}` : ""}${out.unused.length ? ` — unused ${out.unused.join(", ")}` : ""}`);
            } catch (e) {
    ...
    /** The popup's own way to asianwiki: a plain fetch, which an extension page may make. */
