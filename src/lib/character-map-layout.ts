...
export function defaultCompact(map: Pick<CharacterMapData, "main" | "people" | "links">): CharacterMapData["compact"] {
    ...
    // The first two of MDL's main roles; a story with one hero pairs them
    // with whoever has the most ties to them
    const center = map.main.filter((id) => ids.has(id)).slice(0, 2);
    if (center.length < 2) {
    ...
    const tiesTo = (group: string, lead: string | undefined) => {
        if (!lead) return 0;
        const inGroup = new Set(members.get(group) ?? []);
        return ties.filter((l) => (inGroup.has(l.from) && l.to === lead) || (inGroup.has(l.to) && l.from === lead)).length;
    };
    const groups = [...members.keys()]
        .filter((g) => g !== LEADS_GROUP)
        .map((g) => ({ group: g, left: tiesTo(g, center[0]), right: tiesTo(g, center[1]), size: members.get(g)!.length }))
    ...
    for (const g of groups) {
        const leftSide = g.own ? members.get(g.group)!.includes(center[0]) : g.left >= g.right;
