...
export function densityWarnings(map: CharacterMapData): string[] {
    ...
    if (stops.length) {
        ...
        for (const at of stops) {
            ...
            for (const l of holding) for (const id of [l.from, l.to]) if (!leads.has(id)) per.set(id, [...(per.get(id) ?? []), l.short]);
        ...
        for (const [id, { at, shorts }] of heavy) {
            out.push(`${id}: ${shorts.length} ties at once from episode ${at} (${shorts.join(", ")}), at most ${TIES_PER_PERSON_AT_STOP} for someone who is not a lead — a lead to put in compact.center, or a job or a deal for the note`);
        }
    }
    ...
    for (const l of whole) for (const id of [l.from, l.to]) perPerson.set(id, (perPerson.get(id) ?? 0) + 1);
    for (const [id, n] of perPerson) {
        if (!leads.has(id) && n > WHOLE_PER_PERSON) out.push(`${id}: ${n} ties in the whole story, at most ${WHOLE_PER_PERSON} for someone who is not a lead`);
    }
