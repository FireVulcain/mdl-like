...
export async function POST(request: Request) {
    ...
    const body = (await request.json().catch(() => null)) as { mdlSlug?: string; titles?: Record<string, string>; model?: string; recaps?: boolean; mode?: string } | null;
    ...
    const job = await startJob(mdlSlug, userId, titles, model, body?.recaps === true, mode);
