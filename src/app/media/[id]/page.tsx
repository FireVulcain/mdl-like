    ...
    <div>
      ...
      <div className="container relative -top-20 z-10 grid gap-8 md:grid-cols-[300px_1fr] m-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border bg-card p-4 shadow-sm space-y-4 text-sm">
             <div className="grid grid-cols-[100px_1fr] gap-2">
                ...
                <span className="flex items-center gap-2">
                   {media.originCountry} <Badge variant="secondary" className="text-[10px] h-5">{media.originCountry}</Badge>
                </span>
          ...
          <div className="rounded-lg border bg-card p-4 shadow-sm space-y-4">
            ...
            <div className="pt-2">
              <div className="text-sm font-medium mb-2">Episodes</div>
              {userMedia ? (
                 <ProgressTracker 
          ...
          {media.cast && media.cast.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-4">
                 ...
                 <Link href={`/media/${media.id}/cast`} className="text-sm text-primary hover:underline font-medium">
                    View all
                 </Link>
              </div>
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                {media.cast.slice(0, 6).map((actor) => (
                  <div key={actor.id} className="flex-none w-[100px] space-y-2">
                    ...
                    <div>
                      <div className="text-sm font-medium truncate" title={actor.name}>{actor.name}</div>
                      <div className="text-xs text-muted-foreground truncate" title={actor.character}>{actor.character}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {media.images?.backdrops && media.images.backdrops.length > 0 && (
             <div>
                <div className="flex items-center justify-between mb-4">
                   ...
                   <Link href={`/media/${media.id}/photos`} className="text-sm text-primary hover:underline font-medium">
                      View all
                   </Link>
                </div>
